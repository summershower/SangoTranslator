...
export function format(index?: string): string {
  ...
  for (let i = currentActiveSheet.length - 1; i >= 0; i--) {
    ...
    for (let k = 0; k < currentActiveSheet[i].length; k++) {
      if (currentActiveSheet[i][k] === null || !currentActiveSheet[i][k].v)
        nullCount++;
    }
  ...
  for (let row = 0; row < currentActiveSheet.length; row++) {
    for (let column = 0; column < currentActiveSheet[row].length; column++) {
      let value = currentActiveSheet[row][column]?.v;
      if (value) {
        value = value.trim();
  ...
  if (result['en']) {
    result['en'].forEach((v, i) => {
      const wordsArr = v.replace(/[^a-zA-Z0-9 ]/g, ' ').split(' ');
      let currentKey = '';
      for (let i = 0; i < Math.min(8, wordsArr.length); i++) {
        if (!wordsArr[i] || wordsArr[i] === ' ') continue;
        if (i > 0) {
          // 去除多余介词和无意义词语，限制Key值长度
          if (
            ([
              'the',
              'if',
